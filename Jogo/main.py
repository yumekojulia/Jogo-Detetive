...
# A partir daqui eu mudei tudo, ele não puxa mais o jogo pelo terminal, ele puxa um menu que faz o jogador escolher - B
def main():
    import webbrowser  # para abrir links no navegador
    
    clock = pygame.time.Clock()  # Corrigido
    jogadores = []  # Lista de jogadores
    game = None
    game_state = "MAIN_MENU"  # começa no menu principal
    running = True
    input_active = False
    input_text = ""
    
    # Botões do menu principal
    btn_iniciar = pygame.Rect(500, 200, 300, 60)
    btn_instr = pygame.Rect(500, 300, 300, 60)
    btn_refs = pygame.Rect(500, 400, 300, 60)
    btn_sair = pygame.Rect(500, 500, 300, 60)

    # Lista de retângulos clicáveis dos links
    link_rects = []

    # InputBox para nomes (substitui o input no terminal)
    input_box = InputBox(100, 400, 400, 40)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if game_state == "MAIN_MENU":
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    elif btn_refs.collidepoint(event.pos):
                        game_state = "REFERENCES"
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if input_box.text.strip() != "":
                            jogadores.append(input_box.text.strip())
                            input_box.text = ""
                            input_box.txt_surface = font.render("", True, BLACK)
                    elif event.key == pygame.K_SPACE:
                        # Começa o jogo quando tiver pelo menos 2 jogadores
                        if 2 <= len(jogadores) < 6:
                            game = Game(jogadores)
                            game_state = "JOGANDO"
                        else:
                            print("É necessário ter entre 2 e 6 jogadores!")
        ...
        if game_state == "MAIN_MENU":
            ...
            screen.blit(title, (520, 100))

            pygame.draw.rect(screen, GRAY, btn_iniciar)
            screen.blit(font.render("Iniciar", True, WHITE), (btn_iniciar.x+90, btn_iniciar.y+15))

            pygame.draw.rect(screen, GRAY, btn_instr)
            screen.blit(font.render("Instruções", True, WHITE), (btn_instr.x+60, btn_instr.y+15))

            pygame.draw.rect(screen, GRAY, btn_refs)
            screen.blit(font.render("Referências LGPD", True, WHITE), (btn_refs.x+25, btn_refs.y+15))

            pygame.draw.rect(screen, GRAY, btn_sair)
            screen.blit(font.render("Sair", True, WHITE), (btn_sair.x+110, btn_sair.y+15))

        # INSTRUCTIONS
        elif game_state == "INSTRUCTIONS":
            screen.fill(WHITE)
            title = font.render("Regras do Jogo", True, BLACK)
            screen.blit(title, (100, 50))

            regras = [
                "1. O objetivo do jogo é descobrir o culpado em 48 horas!",
                "2. Os jogadores trabalham em equipe se movendo e investigando para chegar a uma solução",
                "3. Ao selecionar o botão Mover é possível se mover para uma casa adjacente (chance de movimento aleatório para qualquer casa)",
                "4. Ao entrar em uma casa é possível Investigar, pegando álibis e inocentando cartas.",
                "5. Se errar a acusação final, o tempo é reduzido em 6 horas.",
                "6. Se o tempo acabar, os investigadores perdem.",
                "7. É possível revisar quem foi inocentado ao clicar em Ver Pistas",
                "8. Se os jogadores acertarem a solução, os investigadores ganham."
            ]
            for i, linha in enumerate(regras):
                txt = font.render(linha, True, BLACK)
                screen.blit(txt, (100, 120 + i*40))

            voltar = font.render("Pressione ESC para voltar", True, RED)
            screen.blit(voltar, (100, 500))

        # REFERENCES
        elif game_state == "REFERENCES":
            screen.fill(WHITE)
            title = font.render("Referências LGPD", True, BLACK)
            screen.blit(title, (100, 50))

            links = [
                ("Lei Geral de Proteção de Dados (Lei 13.709/2018)", "https://www.planalto.gov.br/ccivil_03/_ato2015-2018/2018/lei/l13709.htm"),
                ("Guia Orientativo LGPD (ANPD)", "https://www.gov.br/anpd/pt-br/assuntos/guias-e-orientacoes"),
                ("Cartilha LGPD (Senacon)", "https://www.gov.br/mj/pt-br/assuntos/seus-direitos/lgpd"),
                ("FAQ LGPD (ANPD)", "https://www.gov.br/anpd/pt-br/assuntos/perguntas-frequentes"),
            ]

            link_rects = []
            for i, (label, url) in enumerate(links):
                txt = font.render(label, True, (0,0,255))
                rect = txt.get_rect(topleft=(100, 120 + i*40))
                screen.blit(txt, rect)
                link_rects.append((rect, url))
        ...
        # PLAYER SELECT
            ...
            screen.fill(WHITE)
            title = font.render("Seleção de Jogadores (ESC volta)", True, BLACK)
            screen.blit(title, (100, 50))

            for i, jogador in enumerate(jogadores):
                txt = font.render(f"{i+1}. {jogador}", True, BLACK)
                screen.blit(txt, (100, 120 + i*40))

            # desenha InputBox
            input_box.draw(screen)

            instru = font.render("Digite o nome e pressione ENTER (SPACE para iniciar)", True, RED)
            screen.blit(instru, (100, input_box.rect.y - 30))
